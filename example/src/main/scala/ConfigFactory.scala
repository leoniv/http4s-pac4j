package org.pac4j.http4s.example

import org.pac4j.core.authorization.authorizer.RequireAnyRoleAuthorizer
import org.pac4j.core.authorization.generator.AuthorizationGenerator
import org.pac4j.core.client.Clients
import org.pac4j.core.config.{Config, ConfigFactory => IConfigFactroy}
...
object ConfigFactory extends IConfigFactroy {
  ...
  def oidcClient(): OidcClient[OidcProfile, OidcConfiguration] = {
    ...
    val authorizationGenerator =
      new AuthorizationGenerator[OidcProfile] {
  ...
  def saml2Client() = {
    val cfg = new SAML2Configuration(
      "file:saml-keystore.p12",
      "pac4j-demo-passwd",
      "pac4j-demo-passwd",
      "resource:metadata-okta.xml"
    )
    cfg.setMaximumAuthenticationLifetime(3600)
    cfg.setServiceProviderEntityId(
      "http://localhost:8080/callback?client_name=SAML2Client"
    )
