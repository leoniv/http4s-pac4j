...
object SessionSyntax {
  ...
  implicit final class RequestOps(val v: Request[IO]) extends AnyVal {
    def session: Option[Session] =
      v.attributes.lookup(Session.requestAttr)
  }

  implicit final class ResponseOps(val v: Response[IO])
      extends AnyVal {
    def clearSession: Response[IO] =
      v.withAttribute(
        Session.responseAttr,
        (_: Option[Session]) => None
      )

    def modifySession(f: Session => Session): Response[IO] = {
      val lf: Option[Session] => Option[Session] =
        _.cata(f.andThen(_.some), None)
      v.withAttribute(
        Session.responseAttr,
        v.attributes
          .lookup(Session.responseAttr)
          .cata(_.andThen(lf), lf)
      )
    }

    def newOrModifySession(
      f: Option[Session] => Session
    ): Response[IO] = {
...
object CryptoManager {
  def aes[F[_]: Sync](secret: String): CryptoManager[F] =
    new CryptoManager[F] {
      ...
      private[this] def keySpec: SecretKeySpec =
        new SecretKeySpec(
          secret.substring(0, 16).getBytes("UTF-8"),
          "AES"
        )
      ...
      def encrypt(content: String): F[String] = Sync[F].delay {
        ...
        Hex
          .encodeHex(cipher.doFinal(content.getBytes("UTF-8")))
          .mkString
      }

      def decrypt(content: String): F[Option[String]] =
        Sync[F].delay {
          val cipher = Cipher.getInstance("AES")
          cipher.init(Cipher.DECRYPT_MODE, keySpec)
          Try(
            new String(
              cipher.doFinal(Hex.decodeHex(content)),
              "UTF-8"
            )
          ).toOption
        }
      ...
      def sign(content: String): F[String] = Sync[F].delay {
        ...
        Base64.getEncoder.encodeToString(
          signMac.doFinal(content.getBytes("UTF-8"))
        )
      }
...
final case class SessionConfig[F[_]: Sync](
  ...
  private[this] def decrypt(
    cookie: RequestCookie
  ): F[Option[String]] =
    (for {
      (signature, value) <- OptionT(
        Sync[F].pure(split2(cookie.content.split('-')))
      )
  ...
  def check(cookie: RequestCookie): F[Option[String]] = {
    ...
    (for {
      ...
      (expires, content) <- OptionT(
        Sync[F].pure(split2(decrypted.split("-", 2)))
      )
      expiresSeconds <- OptionT(
        Sync[F].delay(Try(expires.toLong).toOption)
      )
...
object Session {
  ...
  private[this] def checkSignature(
    ...
    OptionT(config.check(cookie))
      .mapFilter(jawn.parse(_).toOption)
      .value
  ...
  private[this] def sessionFromRequest(
    ...
    (for {
      sessionCookie <- OptionT(
        IO.pure(request.cookies.find(_.name === config.cookieName))
      )
  ...
  private[this] def requestWithSession(
    config: SessionConfig[IO],
    request: Request[IO]
  ): IO[(Option[Session], Request[IO])] =
    for {
  ...
  def applySessionUpdates(
    ...
    val updateSession =
      response.attributes.lookup(responseAttr) | identity
    updateSession(sessionFromRequest)
      .map(sessionAsCookie(config, _))
      .sequence
      .map(
        _.cata(
          response.addCookie(_),
          if (sessionFromRequest.isDefined)
            response.removeCookie(config.cookieName)
          else response
        )
      )
  }
  ...
  def sessionManagement(
    ...
    Middleware { (request, service) =>
      for {
        _ <- OptionT.liftF(
          debug(s"starting for ${request.method} ${request.uri}")
        )
        (sessionFromRequest, requestWithSession) <- OptionT.liftF(
          requestWithSession(config, request)
        )
        _ <- OptionT.liftF(
          printRequestSessionKeys(sessionFromRequest)
        )
        ...
        _ <- OptionT.liftF(
          debug(s"finishing for ${request.method} ${request.uri}")
        )
      } yield responseWithSession
    }

  def sessionRequired(
    fallback: IO[Response[IO]]
  ): HttpMiddleware[IO] =
    Middleware { (request, service) =>
  ...
  private[this] def printRequestSessionKeys(
    sessionOpt: Option[Session]
  ) =
    sessionOpt match {
      case Some(session) =>
        debug(
          "Request Session contains keys: " + session.asObject
            .map(_.toMap.keys.mkString(", "))
        )
