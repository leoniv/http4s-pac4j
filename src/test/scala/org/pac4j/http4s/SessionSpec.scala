...
import org.specs2.matcher.{IOMatchers, Matcher}
...
object JsonHelpers {
  implicit val json: EntityDecoder[IO, Json] =
    org.http4s.jawn.jawnDecoder[IO, Json]
  ...
  implicit val jsonEncoder: EntityEncoder[IO, Json] =
    EntityEncoder[IO, String]
      .contramap[Json](json => Printer.noSpaces.pretty(json))
  ...
  implicit def arbSession: Arbitrary[Session] =
    Arbitrary(
      for {
        ...
        .obj("string" -> string.asJson, "number" -> number.asJson)
    )
}
...
object Matchers {
  ...
  def setCookies(resp: Response[IO]): List[Cookie] =
    resp.cookies.filter(
      _.expires.forall(i => !i.toInstant.isBefore(Instant.now()))
    )

  def expiredCookies(resp: Response[IO]): List[Cookie] =
    resp.cookies.filter(
      _.expires.exists(_.toInstant.isBefore(Instant.now()))
    )

  def beCookieWithName(name: String): Matcher[Cookie] =
    be_===(name) ^^ ((_: Cookie).name)

  def beCookieWhoseContentContains(
    subcontent: String
  ): Matcher[Cookie] =
...
class SessionSpec(val exEnv: ExecutionEnv)
  ...
  "session management" should {
    def sut: HttpApp[IO] =
      Session
        .sessionManagement(config)(HttpService {
          case GET -> Root / "id" =>
            Ok()
          ...
          case GET -> Root / "modify" =>
            val _number =
              jsonObject ^|-> at[JsonObject, String, Option[Json]](
                "number"
              ) ^<-? Monocle.some ^<-? jsonInt
    ...
    "Doing nothing" should {
      "not clear or set a session cookie when there is no session" in {
        ...
        sut(request) must returnValue(
          not(
            haveSetCookie(config.cookieName)
              .or(haveClearedCookie(config.cookieName))
          )
        )
      }

      "not clear a session cookie when one is set" in prop {
        session: Session =>
          config
            .cookie(session.noSpaces)
            .map(
              cookie =>
                Request[IO](Method.GET, uri"/id")
                  .addCookie(cookie.toRequestCookie)
            )
            .flatMap(sut(_)) must returnValue(
            not(haveClearedCookie(config.cookieName))
          )
      }
    }

    "Creating a session" should {
      "set a session cookie as per mkCookie" in prop {
        session: Session =>
      ...
      "not include the session data in a readable form in the cookie" in {
        ...
        sut(request).map(setCookies _) must
          returnValue(
            not(contain(beCookieWhoseContentContains("created")))
          )
      }
    }

    "Clearing a session" should {
      "clear session cookie when one is set" in prop {
        session: Session =>
          config
            .cookie(session.noSpaces)
            .map(
              cookie =>
                Request[IO](Method.GET, uri"/clear")
                  .addCookie(cookie.toRequestCookie)
            )
            .flatMap(sut(_)) must returnValue(
            haveClearedCookie(config.cookieName)
          )
      }
    ...
    "Reading a session" should {
      ...
      "read None when the session is signed with a different secret" in prop {
        session: Session =>
          config
            .copy[IO](cryptoManager =
              CryptoManager.aes("this is a different secret")
            )
            .cookie(session.noSpaces)
            .map(
              cookie =>
            ...
            .flatMap(sut(_)) must returnValue(
            haveStatus(Status.NotFound)
          )
      }

      "read None when the session has expired" in prop {
        session: Session =>
          config
            .copy[IO](maxAge = 0.seconds)
            .cookie(session.noSpaces)
            .map(
              cookie =>
                Request[IO](Method.GET, uri"/read")
      ...
      "read the session when it exists" in prop { session: Session =>
        config
          ...
          .flatMap(sut(_)) must returnValue(
          haveStatus(Status.Ok).and(haveBody(session))
        )
      }
    }
      ...
      "update the session when set" in {
        ...
        val response = for {
          cookie <- config.cookie(
            Json.obj("number" -> 0.asJson).noSpaces
          )
          ...
          secondRequest = cookies.foldLeft(
            ...
            (r: Request[IO], c: ResponseCookie) =>
              r.addCookie(c.toRequestCookie)
          )
          secondResponse <- sut(secondRequest)
        } yield secondResponse
        response must returnValue(
          haveBody(Json.obj("number" -> 1.asJson))
        )
      }
  ...
  "session required" should {
    ...
    def sut: HttpApp[IO] =
      Session
        ...
        .compose(Session.sessionRequired(fallback))(HttpService {
          case GET -> Root =>
            Ok()
        })
        .orNotFound

    "allow access to the service when a session is set" in prop {
      session: Session =>
        config
          .cookie(session.noSpaces)
          .map(
